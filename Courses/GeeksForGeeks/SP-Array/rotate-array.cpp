...
VI a(1000 * 10000 + 10, 0);

int main( void ) {

#ifdef DEBUG
  freopen("rotate-array.in", "r", stdin);
#endif

  
  int T; cin >> T; while (T--) {
    int n; cin >> n; int d; cin >> d; REP(i, n) cin >> a[i]; 
    vector<bool> mr(n, false);
    REP(i, n) {
      int cur = a[i], cur_pos = i;
      while (!mr[cur_pos]) {
        mr[cur_pos] = true;
        int ne_pos = (cur_pos - d +n) % n, ne = a[ne_pos];
        a[ne_pos] = cur; cur_pos = ne_pos; cur = ne;
      }
    }
    REP(i, n) PRC(a[i]); NL;
  }
