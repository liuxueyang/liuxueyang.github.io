...
mat mul(mat x, mat y) {
  ...
  REP(i,3) {                  // row
    REP(j,3) {
      ...
      REP(k, 3) {
        res.a[i][j] = ((
                        ((x.a[i][k]%MOD) * (y.a[k][j]%MOD)) %MOD) + res.a[i][j]%MOD )%MOD;
      }
    }
  }
  return res;
  ...
  while(cin>>L>>A>>B>>MOD) {
    ...
    REP_1(i,19) {
      ...
      if (k/10 < A) {
        ...
        cd = ceil(1.0 * (k - A) / B) - ceil(1.0 * (k / 10 - A) / B);
      }
      if(cd<=0) continue;
      ...
      REP(i,3) REP(j,3)
        v1[i]=(v1[i]%MOD+ (v[j]%MOD)*(re.a[j][i]%MOD) %MOD)%MOD;
