...
int solve(VI &a) {
  int len = a.size();
  VI leftv(a.size(), 0), rightv(a.size(), 0);
  leftv[0] = a[0]; rightv[len-1] = a[len-1];
  int leftm = a[0]; REP(i, len) {
    leftm = max_(leftm, a[i]); leftv[i] = leftm;
  }
  int rightm = a[len-1]; REPV(i, len) {
    rightm = max_(rightm, a[i]); rightv[i] = rightm;
  }
  int res=abs(rightv[1]-leftv[0]);
  REP(i, len-1) {res = max_(res, abs(rightv[i+1] - leftv[i]));}
  return res;
}
...
int solve2(VI &a) {
  ...
  int maxv = a[0]; REP(i, len) maxv = max_(maxv, a[i]);
  return max_(abs(maxv - a[0]), abs(maxv - a[len-1]));
}

int main( void ) {

#ifdef DEBUG
  freopen("left-right-max.in", "r", stdin);
#endif
