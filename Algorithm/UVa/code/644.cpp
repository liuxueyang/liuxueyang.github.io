...
int main( void ) {
  ...
  while (cin >> s) {
    if (s == "9") {
      ...
      REP(i, codes.size()) {
        REP(j, codes.size()) {
          if (codes[i].size() >= codes[j].size()) continue;
          if (codes[i] == codes[j].substr(0, codes[i].size())) { ok = 1; break;}
        }
        if (ok) break;
      }
      PRLN(ok ? "is not immediately decodable" : "is immediately decodable");
      while (!codes.empty()) codes.pop_back();
      cnt++;
    }
    codes.push_back(s);
  }
