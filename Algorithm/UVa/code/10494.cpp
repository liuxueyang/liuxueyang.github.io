...
int main( void ) {
  ...
  while (getline(cin, line)) {
    ...
    while (pos < a.size()) {
      ...
      int cnt = 1;
      bign cnt_(cnt), pre_cnt_ = cnt_, pro = cnt_ * bb;
      ...
      div_res += pre_cnt_.str();
      bign rem = up_ - pre_cnt_ * bb;
      up = rem.str();
    }

    int i = 0; while (i < div_res.size() && '0' == div_res[i]) i++;
    // PR1(i);
    if (i < div_res.size())
      div_res = div_res.substr(i);
    else div_res = "0";

    i = 0; while (i < up.size() && '0' == up[i]) ++i;
    if (i < up.size()) up = up.substr(i);
    else up = "0";
    
    rem_res = up == "" ? "0" : up;
    div_res = div_res == "" ? "0" : div_res;
