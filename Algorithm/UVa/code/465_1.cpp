...
int main( void ) {
  ...
  string a, op, b, line;
  BNI maxn = LL(numeric_limits<int>::max()), res;
  
  while (getline(cin, line)) {
    stringstream ss(line);
    ss >> a >> op >> b;
    res = (op == "+" ? BNI(a) + b : BNI(a) * b);

    PRLN(line);
    // PR2(maxn, a);
    if (maxn < a) PRLN("first number too big");
    if (maxn < b) PRLN("second number too big");
    if (maxn < res) PRLN("result too big");
  }
