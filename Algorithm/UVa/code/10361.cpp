...
int main( void ) {
  ...
  using S = string;
  S l1, l2, c2, s, s2, s3, s4, s5;
  int n;
  while (getline(cin, s)) {
    SS ss(s); ss >> n;
    REP(i, n) {
      getline(cin, l1); getline(cin, l2);
      for (auto x : l1) if (x != '<' && x != '>') cout << x;
      NL;
      size_t p1, p2, p3, p4;
      p1 = l1.find("<"); p2 = l1.find(">");
      s2 = l1.substr(p1 + 1, p2 - p1 - 1);
      
      p3 = l1.find("<", p2);
      s3 = l1.substr(p2 + 1, p3 - p2 - 1);
      
      p4 = l1.find(">", p3);
      s4 = l1.substr(p3 + 1, p4 - p3 - 1);

      s5 = l1.substr(p4 + 1);

      c2 = l2; int len = c2.length();
      c2.replace(len - 3, 3, s4 + s3 + s2 + s5);
      PRLN(c2);
    }
  }
