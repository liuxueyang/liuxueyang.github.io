...
int main( void ) {
  ...
  while (getline(cin, s)) {
    stringstream ss(s); ss >> k >> e;
    REP(i, k) {
      string s1;
      getline(cin, s1);
      stringstream ss1(s1); ss1 >> s;
      keyword.insert(s);
    }
    REP(i, e) {
      getline(cin, s); sen.push_back(s);
      string t = s;
      REP(j, t.length())
        if (!isalpha(t[j])) t[j] = ' ';
        else t[j] = tolower(t[j]);
      int c = 0; stringstream s(t); string tmp;
      while (s >> tmp) {
    ...
    cout << "#" << cast++ << endl;
    REP(i, e) {
      if (most == cnt[i]) PRLN(sen[i]);
    }
    while (!sen.empty()) sen.pop_back(), cnt.pop_back();
    keyword.clear();
    most = 0;
    NL;
  }
