...
int main( void ) {
  ...
  while (getline(cin, line)) {
    ...
    string r; int n;
    ss >> r >> n; int pos = -1;
    string t; REP(i, r.length()) if (r[i]!='.') t+=r[i]; else pos=i;
    pos=5-pos;
    bign R(t.c_str()), bR(t.c_str());
    REP(i, n - 1) { R = R * bR; }
    pos *= n;
    // PRLN(R);
    // PR2(pos, R.len);
    ...
    if (pos < R.len) {
      DWN(i, R.len, pos) cout<<(R.s[i]);
      cout<<".";
      // don't use printf and cout at the same time.
      // printf(".");
      int strt=0;
      while(!R.s[strt]) strt++;
      // delete tail 0s
      DWN(i, pos, strt) cout<<(R.s[i]);
    }
    else {
      pos -= R.len;
      cout<<".";
