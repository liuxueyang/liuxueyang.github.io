...
int main( void ) {
  ...
  while (casn--) {
    ...
    string s, t;
    REP(i, m) {
      cin >> s; t = s; REP(j, t.length()) t[j] = tolower(t[j]);
      grid.push_back(t);
    }
    int k; cin >> k;
    while (k--) {
      int resx = -1, resy = -1, ok = 0;
      cin >> s; string t = s; REP(j, t.length()) t[j] = tolower(t[j]);
      REP(i, m) {
        REP(j, n) {
          int k, dx, dy;
          REP(del, 8) {
            dx = i, dy = j; k = 0;
            while (dx >= 0 && dx < m && dy >= 0 && dy < n && k < t.size() && grid[dx][dy] == t[k]) {
              dx += dir[del][0]; dy += dir[del][1]; k++;
            }
            if (k == t.size()) { resx = i, resy = j; ok = 1; break;}
          }
          if (ok) break;
        }
        if (ok) break;
