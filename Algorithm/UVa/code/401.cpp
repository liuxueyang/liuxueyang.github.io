...
int main( void ) {
  ...
  string ori, re;
  ori = "AEHIJLMOSTUVWXYZ12358";
  re  = "A3HILJMO2TUVWXY51SEZ8";
  map<char, char> mcc;
  REP(i, ori.length()) mcc[ori[i]] = re[i];
  string s;
  while (cin >> s) {
    bool pali = false, mirr = false;
    string t = s;
    reverse(t.begin(), t.end()); pali = (t == s);
    string ms;
    for (auto x : t) {
      if (mcc.find(x) != mcc.end()) {
        ms += mcc[x];
      }
      else ms += ' ';
    }
    mirr = (ms == s);
    PRC(s);
    if (mirr && pali) PRLN("-- is a mirrored palindrome.");
    else if (pali) PRLN("-- is a regular palindrome.");
    else if (mirr) PRLN("-- is a mirrored string.");
    else PRLN("-- is not a palindrome.");
    NL;
  }
