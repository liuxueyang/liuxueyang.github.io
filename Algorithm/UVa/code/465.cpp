...
int main( void ) {
  ...
  bign imax (numeric_limits<int>::max());
  string a, op, b, line;
  ...
  while (getline(cin, line)) {
    ...
    ss >> a >> op >> b;
    bign biga(a.c_str()), bigb(b.c_str());
    bign bigc;
    
    if (op == "+") bigc = biga + bigb;
    else bigc = biga * bigb;

    // PR1(bigc);
    
    if (biga > imax) PRLN("first number too big");
    if (bigb > imax) PRLN("second number too big");
    if (bigc > imax) PRLN("result too big");
  }
