...
int main( void ) {
  ...
  // https://uva.onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&category=96&problem=1819&mosmsg=Submission+received+with+ID+21947388
  ...
  while(NULL != fgets(s, 20, stdin)) {
    int c = 6, res = 0, ok = 0;

    for (int i = 2; s[i] != '\n' && s[i]; ++i) {
      if (s[i] == '.') continue;
      if (s[i] == 'o') { res += (1 << c); ok = 1;}
      c--;
    }
