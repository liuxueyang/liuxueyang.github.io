...
int main( void ) {
  ...
  while(cin>>n>>l>>r){
    cnt[0]=floor(r*1.0/3)-ceil(l*1.0/3)+1;
    cnt[1]=floor((r+1)*1.0/3)-ceil((l+1)/3.)+1;
    cnt[2]=floor((r+2)/3.)-ceil((l+2)/3.)+1;
    memset(dp,sizeof(dp),0);
    REP(i,3)dp[0][i]=cnt[i];
    FOR_1(i,1,n-1){
      dp[i][0]=mul_(dp[i-1][0],cnt[0])+
        mul_(dp[i-1][1],cnt[2])+
        mul_(dp[i-1][2],cnt[1]);
      dp[i][1]=mul_(dp[i-1][0],cnt[1])+
        mul_(dp[i-1][1],cnt[0])+
        mul_(dp[i-1][2],cnt[2]);
      dp[i][2]=mul_(dp[i-1][0],cnt[2])+
        mul_(dp[i-1][1],cnt[1])+
        mul_(dp[i-1][2],cnt[0]);
      REP(j,3)dp[i][j]%=MOD;
    }
