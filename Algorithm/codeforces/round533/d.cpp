...
void dfs(int ag, int x, int y, int step) {
  vis[x][y]=true;
  if (step<=0) {
    vis[x][y]=false;
    return;
  ...
  REP(i, 4) {
    ...
    if(check(nx,ny)){
      if (ar[nx][ny]==0) {      // 空格子一定要走
        ar[nx][ny] = ag + rounds * 10;
        sr[nx][ny]=step-1;
        flag=true;
        checked--;
        if(0==checked)return;
        dfs(ag, nx, ny, step-1);
      } else if(ar[nx][ny]%10==ag
                // &&ar[nx][ny]<ag+rounds*10
                &&sr[nx][ny]<step-1
                ) {
        // 需要记录额外信息，比如到达這個位置還剩下幾步
        // PRC("reentering:"); PR2(nx,ny);
        ar[nx][ny] = ag + rounds * 10;
        sr[nx][ny]=step-1;
        dfs(ag,nx,ny,step-1);
      }
...
int main( void ) {
  ...
  while (cin>>n>>m>>p) {
    ...
    memset(sr,sizeof(sr),0);

    REP(i,n)REP(j,m){sr[i][j]=ar[i][j]=0,vis[i][j]=false;}
    
    checked=0;
    rounds=0;

    // REP(i,n)
    //   {REP(j,m) PRC(sr[i][j]); NL;}
    
    
    REP(i, n)
      REP(j, m)
      if (a[i][j] == '.') {ar[i][j] = 0; checked++;
        // if (i==2) PR2(a[i][j], sr[i][j]);
      }
      else if (a[i][j] == '#') ar[i][j] = -1;
      else { ar[i][j] = a[i][j] - '0'; sr[i][j]=s[ar[i][j]-1];
        // if (i==2) PR3(a[i][j], ar[i][j], sr[i][j]);
      }
    ...
    while(checked&&flag){
      ...
      REP_1(i, p) {
        ...
        REP(ii, n)
          REP(jj, m) {
          if(ar[ii][jj] %10==i&&ar[ii][jj]<rounds*10+i){
            // PR2(ii,jj);
            dfs(i,ii,jj,s[i-1]);
          }
        }
