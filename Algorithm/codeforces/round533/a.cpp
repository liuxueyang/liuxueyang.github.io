...
int cal_cost(int n, int ave) {
  int res = 0;
  REP(i, n) {
    if (a[i] < ave - 1) res += (ave - 1 - a[i]);
    else if (a[i] > ave + 1) res += (a[i] - ave - 1);
  }
  return res;
}
...
int main( void ) {
  ...
  while (cin >> n) {
    ...
    // int ave = sum / n;
    // vector<pair<int,int>> v {make_pair(cal_cost(n,ave-1), ave-1),
    //                          make_pair(cal_cost(n,ave), ave),
    //                          make_pair(cal_cost(n,ave+1), ave+1)};
    
    vector<PII> v; REP_1(i, 100) { v.push_back(make_pair(cal_cost(n, i), i)); }
    sort(v.begin(), v.end());
    
    PRC(v[0].second); PRLN(v[0].first);
  }
