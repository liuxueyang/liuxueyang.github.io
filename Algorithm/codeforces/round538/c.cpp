...
// prime table
const int PRIME_MAX = 1000 * 1000 + 10;
VI prime_table;
...
int main( void ) {
  ...
  while(cin>>n>>b) {
    ...
    int psize=prime_table.size();
    PRLN(psize);
    REP(i,psize){
      if(prime_table[i]>b)break;
      auto x=ex(prime_table[i],b);
      cnt[i]=x.first;
      b=x.second;
    }
    if(b>1) {
      prime_table[psize-1]=b;
      cnt[psize-1]=1;
    }
    LL res=numeric_limits<LL>::max();
    REP(i,psize){
      if(cnt[i]){
        LL c=ex1(prime_table[i],n);
        res=min_(res,c/cnt[i]);
      }
    }
    PRLN(res);
  }
