...
int main( void ) {
  ...
  int n,m,k;
  while(cin>>n>>m>>k){
    REP(i,n){cin>>a[i];v[i]=make_pair(a[i],i);us[i]=false;}
    sort(v,v+n);//,[](PII&a1,PII&b1){return a1.first<=b1.first;});
    // REP(i,n)PR2(v[i].first,v[i].second);
    // PRLN("");
    LL res=0;
    FOR_1(i,n-k*m,n-1){us[v[i].second]=true;res+=v[i].first;}
    PRLN(res);
    int cnt=0, ans=0;
    REP(i,n){
      if(us[i])cnt++;
      if(cnt==m&&ans!=k-1){cnt=0;PRC(i+1);ans++;}
    }
    NL;
  }
