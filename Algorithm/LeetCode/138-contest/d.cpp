...
class Solution {
 public:
  vector<int> rearrangeBarcodes(vector<int>& barcodes) {
    ...
    MII cntn;
    REP(i,len) cntn[barcodes[i]]++;
    sort(barcodes.begin(),barcodes.end(), [&](int x,int y){ return cntn[x] < cntn[y]; });

    VI ppp;
    SI ppps;
    REP(i, len){
      if (ppps.find(barcodes[i])==ppps.end()){
        ppps.insert(barcodes[i]);
        ppp.push_back(barcodes[i]);
      }
    }
    ...
    REP(i, cntn[item]) {
      vvi.push_back(VI{item});
    }
    for(auto item=++ppp.begin();item!=ppp.end();++item){
      REP(i, cntn[item]) vvi[i].push_back(item);
    }
    REP(i,vvi.size()){
      REP(j,vvi[i].size()){
        res.push_back(vvi[i][j]);
      }
    }
    return res;
  }
...
int main( void ) {
  ...
  VI bar1{1,1,1,2,2,2},
      bar2{1},
      bar3{2,2,1,3},
      bar4{7,7,7,8,5,7,5,5,5,8};
  PRCO(a.rearrangeBarcodes(bar1));
  PRCO(a.rearrangeBarcodes(bar2));
  PRCO(a.rearrangeBarcodes(bar3));
  PRCO(a.rearrangeBarcodes(bar4));
