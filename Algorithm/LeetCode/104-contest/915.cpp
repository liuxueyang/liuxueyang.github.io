...
public:
  int partitionDisjoint(vector<int>& A) {
    int len = A.size();
    VI left_max(len), right_min(len);
    left_max[0] = A[0]; right_min[len-1] = A[len-1];
    FOR(i, 1, len) { left_max[i] = max_(left_max[i-1], A[i]); }
    DWN(i, len - 1, 0) { right_min[i] = min_(right_min[i+1], A[i]); }
    REP(i, len - 1) { if (left_max[i] <= right_min[i+1]) return i + 1; }
    return 0;
  }
};
...
int main( void ) {
  ...
  Solution s;
  for (auto &x : a) { PRLN(s.partitionDisjoint(x)); }
