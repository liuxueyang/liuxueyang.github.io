...
public:
  ...
  void trace(ptn root,int ma, int mi) {
    if(root==NULL) return;
    ptn le=root->left, ri=root->right;
    if(le!=NULL){
      int left_ma = max_(ma, le->val), left_mi = min_(mi, le->val);
      res=max_(res,abs(left_ma-left_mi));
      trace(le,left_ma, left_mi);
    }
    if(ri!=NULL){
      int right_ma =max_(ma, ri->val), right_mi=min_(mi, ri->val);
      res=max_(res,abs(right_ma-right_mi));
      trace(ri,right_ma,right_mi);
    }
  }
};
