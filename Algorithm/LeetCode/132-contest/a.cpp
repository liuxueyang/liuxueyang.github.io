...
public:
  Solution() {
    for(auto x:prime_table) prs.insert(x);
    FOR_1(i,2,1003){
      if(prs.find(i)!=prs.end()) check[i]=!check[i-1];
      else{
        bool f=false;
        FOR_1(j,1,ceil(sqrt(i))) {
          if(i%j==0&&!check[i-j]) check[i]=true, f=true; break;
        }
        if(!f) check[i]=false;
      }
    }
