...
public:
  ...
  bool match(string query, string pattern) {
    VS qs = split_str(query), ps = split_str(pattern);
    if (qs.size() != ps.size()) return false;
    REP(i, qs.size()) {
      if (!is_prefix(ps[i], qs[i])) return false;
    }
    return true;
  }
  VS split_str(string s) {
    VS res;
    REP(i, s.size()) {
      while (i < s.size() && is_lower(s[i])) ++i;
      string t;
      if (i < s.size()) t += s[i++];
      while (i < s.size() && is_lower(s[i])) t += s[i++];
      --i;
      res.push_back(t);
    }
    return res;
