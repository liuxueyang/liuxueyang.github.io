...
public:
  int maxSumTwoNoOverlap(vector<int>& A, int L, int M) {
    vector<PII> vl(A.size() - L + 1), vm(A.size() - M + 1);
    REP(i, A.size() - L + 1) {
      int sum=0;
      FOR(j, i, i+L) sum += A[j];
      vl.push_back(MK(i, sum));
    }
    REP(i, A.size() - M + 1) {
      int sum=0;
      FOR(j, i, i+M) sum += A[j];
      vm.push_back(MK(i, sum));
    }
    int res=-1;
    REP(i, vl.size()) {
      REP(j, vm.size()) {
        int i1 = vl[i].first + L - 1, j0 = vm[j].first;
        if (i1 < j0) {
          res=max_(res, vl[i].second + vm[j].second);
        }
        int i0 = vl[i].first, j1 = vm[j].first + M  -1;
        if (j1 < i0) {
          res=max_(res, vl[i].second + vm[j].second);
        }
      }
