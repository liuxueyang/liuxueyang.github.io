...
public:
  ...
  BNF operator+(const BNF &rhs) const {
    ...
    string inte1 = in_fr1.first, frac1 = in_fr1.second, inte2 = in_fr2.first,
           frac2 = in_fr2.second;
    LL inc = 0LL;
    BNI frac_sum;
  ...
  BNF operator-(const BNF &rhs) const {
    ...
    while (frac1.size() < frac2.size())
      frac1 += '0';
    while (frac2.size() < frac1.size())
      frac2 += '0';
    BNI frac = BNI(frac1) - frac2;
    BNI inte = BNI(inte1) - dec - inte2;
    return join_inte_frac(inte, frac);
  }
  BNF &operator-=(const BNF &rhs) { return *this = *this - rhs; }
  BNF operator*(const BNF &rhs) const {
    pair<string, string> in_fr1 = split(), in_fr2 = rhs.split();
    string inte1 = in_fr1.first, frac1 = in_fr1.second, inte2 = in_fr2.first,
           frac2 = in_fr2.second;
...
public:
  int longestStrChain(vector<string>& words) {
    ...
    for(int i=0;i<len;) {
      while (words[i].size()==ps) {
        ind.insert(i);
        ++i;
      }
      SI nnd;
      while (words[i].size()==ps+1) {
        if (check(i, ind, words)) {
          nnd.insert(i);
        }
        ++i;
      }
      if (nnd.size()) res=max_(ps+1, res);
      else { res=max_(ps, res); break; }
      ps++;
      ind.clear();
    }
    return res;
  }
  bool check(int i, SI ind, VS words) {
    string tar=words[i];
    for(auto x: ind) {
      string ps = words[x];
      if (check(ps, tar)) return true;
    }
    return false;
  }
  bool check(string w1, string w2) {
    if (w1.size()+1!=w2.size()) return false;
    VC wi1(w1.size()), wi2(w2.size());
    for (int i=0; i<w1.size(); ++i) wi1[i]=w1[i];
    for (int i=0; i<w2.size(); ++i) wi2[i]=w2[i];
    sort(wi1.begin(), wi1.end());
    sort(wi2.begin(), wi2.end());
    int mark=0, off=0;
    for (int i=0; i<w1.size(); ++i) {
      if (!off) {
        if (w1[i]!=w2[i]) off=1;
      } else {
        if (w1[i]!=w2[i+off]) return false;
      }
    }
    return true;
  }
