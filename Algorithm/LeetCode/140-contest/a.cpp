...
class Solution {
 public:
  VS findOcurrences(string text, string first, string second) {
    VS res;
    stringstream s(text);
    string tmp;
    int cnt=0;
    ...
    while (s >> tmp) {
      PR1(tmp);
      if (tmp==first && cnt==0) cnt++;
      else {
        if (tmp==second && cnt==1) cnt++;
        else if (cnt==2) {
          res.push_back(tmp);
          cnt=0;
          if (tmp==first && cnt==0) cnt++;
...
int main( void ) {
  ...
  Solution a;
  auto r = a.findOcurrences("j j k j k j k k j k", "k", "j");
  PRCO(r);
