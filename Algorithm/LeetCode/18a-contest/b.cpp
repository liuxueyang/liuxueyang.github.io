...
public:
  ...
  vector<int> findFrequentTreeSum(TreeNode* root) {
    VI res; int max_c = INT_MIN;
    if(root==NULL) return res;
    postroot(root);
    for(auto x:cnt){
      max_c=max_(max_c,x.second);
    }
    for(auto x:cnt){
      if (x.second==max_c) res.push_back(x.first);
    }
    return res;
  }
  int postroot(TreeNode* root) {
    ...
    right=root->right;
    int val=root->val;
    if(left==NULL&&right==NULL) {cnt[val]++;return val;}
    int left_val = postroot(left), right_val = postroot(right);
    int sum=left_val+right_val+val;
    cnt[sum]++;
    return sum;
  }
};
