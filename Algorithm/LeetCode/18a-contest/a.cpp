...
public:
  vector<string> findWords(vector<string>& words) {
    VS al {"qwertyuiop", "asdfghjkl", "zxcvbnm"};
    vector<set<char>> vs(3);
    REP(i,3) {
      REP(j, al[i].length()) {vs[i].insert(al[i][j]); vs[i].insert(toupper(al[i][j]));}
    }
    VS res;
    for(auto x:words) {
      if (x.length()==0) { res.push_back(x); continue; }
      int pos=-1;
      bool mark=true;
      REP(i,3) { if (vs[i].find(x[0])!=vs[i].end()) {pos=i; break;} }
      REP(i, x.length()) {
        if (vs[pos].find(x[i])==vs[pos].end()) {
          mark=false;break;
        }
      }
      if(mark) res.push_back(x);
    }
    return res;
  }
};
...
int main( void ) {
  ...
  Solution sol;
  VS vs{"Hello", "Alaska", "Dad", "Peace"};
  VS res = sol.findWords(vs);
  PRCO(res);
